...
2. The operating doctrine lives in `plugins/arion-company/skills/<name>/SKILL.md` (37 skills). Before doing specialist work (native app, UI/UX, security, data, DevOps, outbound), open the matching skill file and follow it — the routing map is `plugins/arion-company/skills/orchestration-runpack/SKILL.md`.
3. Personas live in `plugins/arion-company/agents/<team>/<role>.md`. One owner per task; teams not routed to the task stay silent.
...
- **Claude Code**: full experience — commands (`/onboard`, `/build`, `/design`, `/schema`, `/fix`, `/team`, `/company`) come from the plugin (`/plugin install arion-company@arion`). Works on an Anthropic API key or a Claude Pro/Max subscription login.
